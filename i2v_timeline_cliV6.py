...
- Every garment named in "appearance" must specify its silhouette, not just its name: sleeve length for tops (sleeveless/short-sleeve/long-sleeve), and hem length for skirts/dresses (mini/knee-length/midi/floor-length). A bare garment name like "crop top" or "skirt" leaves the silhouette undefined — since each segment's keyframe image is generated independently, an unspecified silhouette renders inconsistently from segment to segment. Always pin it down explicitly (e.g. "short-sleeve white crop top, knee-length pleated skirt").
...
_KF8_RULE = (
    'KF8 SELFIE ALREADY IMPLIES THE PHONE: "selfie" by itself already means she is holding up her '
    'phone to photograph herself — do NOT additionally write "holding a smartphone", "holding her '
    'phone", "gripping her phone", etc. as a separate action. This double-describes the same phone '
    'and causes a third hand to be generated. Remove the redundant explicit phone-holding phrase '
    'and keep "selfie" alone.'
)
...
async def _format_keyframe_prompt(scene_desc: str, global_desc: str, duration: int, orientation: str, direction: str = "", character_line: str = "", action: str = "", reserved_props: list[str] | None = None, style_tail: str = _DEFAULT_STYLE_TAIL, state: str = "") -> str:
    ...
    def _kf2_detect(text: str) -> list[str]:
        ...
        # KF8: "selfie"がスマホを持つ行為を既に含意するのに、別途「holding a smartphone」等の
        # 明示描写が同居 → 暗黙+明示の2つのスマホ/3本目の手が生成される(2026-07-16)
        if _selfie_phone_redundancy(text):
            rules.append(_KF8_RULE)
