...
1. **Python環境**: このリポジトリ直下に `venv` という名前の仮想環境を作る(**フォルダ名・配置場所は固定**——
   ...
   `uv`を使う場合は `uv venv venv` でも同じ構造の仮想環境ができるため、以降の手順は共通。
...
- **依存パッケージはリポジトリ直下の`venv`に追加インストールするだけでよい**(専用の仮想環境は不要。`requirements.txt`と`CASS/requirements.txt`の間にパッケージの重複・バージョン衝突が無いことを確認済み): `venv`を有効化した状態で`pip install -r CASS/requirements.txt`。セットアップ手順・オプション・ライセンスは [CASS/README.md](CASS/README.md) 参照
...
**thinking(reasoning)モデルを使う場合の注意**: DeepSeek系など「思考過程」を出力するモデルを`LLM_MODEL`に指定すると、system promptが長いパス(特にPass1のショットディレクター)でreasoning側のトークン消費が実応答より大きくなり、生成が大幅に遅くなったり、`max_tokens`予算を思考だけで使い切って本文が空になったりすることがあります。可能な限り、サーバー側(vLLM・LM Studio・SGLang等)の設定でthinkingをOFFにした状態で使うことを推奨します。このプロジェクトのコード側でも主要なOpenAI互換パラメータ(`thinking: {"type": "disabled"}`)を全LLM呼び出しに付与していますが、対応していないサーバー実装では効かない場合があります。
